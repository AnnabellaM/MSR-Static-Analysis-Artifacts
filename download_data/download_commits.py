...
repos = repos[repos['Repository'].isin([
    "spotbugs/spotbugs",])]
...
for _, repo in repos[repos['Host'] == "Github"].iterrows():
    ...
    for commit in tqdm(commits, desc=repo['Repository'], total=commits.totalCount):
        if os.path.exists(f'../data/commits/Github_{repo["Repository"].split("/")[1]}_{commit.sha}.json'):
        ...
        json.dump(commit.raw_data, open(f'../data/commits/Github_{repo["Repository"].split("/")[1]}_{commit.sha}.json', 'w')) 
...
for _, repo in repos[repos['Host'] == "Bitbucket"].iterrows():
    ...
    while True:
        ...
        for commit in commits['values']:
            if os.path.exists(f'../data/commits/Bitbucket_{repo["Repository"].split("/")[1]}_{commit["hash"]}.json'):
            ...
            json.dump(data, open(f'../data/commits/Bitbucket_{repo["Repository"].split("/")[1]}_{commit["hash"]}.json', 'w'))
